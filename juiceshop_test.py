...
class TestOWASPJuiceShop(unittest.TestCase):
    ...
    def test_weak_password_requirements(self):
        ...
        payload = { # Payload for a new unique user (must change each run)
            "email": "test20222222222222222222222222222022222@test.com",
            "password": "12345",
    ...
    def test_improper_input_validation(self):
        ...
        #id_value = json_data['status']
        #print(json_data)

       # response = requests.put(url+f"/api/BasketItems/58", headers=adminHeaders, json={"quantity": -10})
       # self.assertNotEqual(response.status_code, 200)
